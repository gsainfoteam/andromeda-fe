...
const Star = (props: StarProps) => {
  ...
  const calcStarRates = () => {
      ...
      let idx = 0;
      while (starVerScore > 576) { // 별 하나하나에 채워질 width를 지정
  ...
  useEffect(() => { setRatesResArr(calcStarRates) }, [])
  const DefColor = props.color == 'green' ? `${colorSet.galactic_green}` : `${colorSet.galactic_purple}`
    ...
    <StarRateWrap>
      {STAR_IDX_ARR.map((item, idx) => {
        return <span className='star_icon' key={`${item}_${idx}`}>
          <svg
            xmlns='hhttp://www.w3.org/2000/svg' width={props.width} height={props.width} viewBox='0 0 576 512' fill={DefaultColor} >
